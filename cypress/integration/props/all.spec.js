...
describe("from and to props", () => {
	...
	it("time - with click", () => {
		...
		cy.get(".pdp-moment button")
			.eq(1)
			.as("momentButtonOne");
		cy.get(".pdp-moment button")
			.eq(3)
			.as("momentButtonThree");
		cy.get(".pdp-moment button")
			.eq(4)
			.as("momentButtonFour");
		cy.get(".pdp-moment button")
	...
	it("datetime - with click", () => {
		...
		cy.visit("/");
		cy.get(".pdp-input").focus();
		cy.get('.pdp-day[value="1"]')
			.first()
			.click();
		cy.get('.pdp-day[value="31"]')
			.first()
			.click();
		cy.get(".pdp-input").focus();
		cy.get(".pdp-moment button")
		...
		cy.get(".pdp-input").should(
			"have.value",
			"1399/06/01 10:10 - 1399/06/31 20:20"
		);
	});
...
describe("alternative field", () => {
	...
	it("exist - Array", () => {
		...
		cy.get('input[type="hidden"]')
			.first()
			.should("have.value", "20-8-31");
		cy.get('input[type="hidden"]')
			.last()
			.should("have.value", "20-9-5");
	});
	...
	it("without alt-format", () => {
		...
		cy.get('input[type="hidden"]')
			.first()
			.should("have.value", "2020 08");
		cy.get('input[type="hidden"]')
			.last()
			.should("have.value", "2020 09");
	});
	...
	it("with alt-format", () => {
		...
		cy.get('input[type="hidden"]')
			.first()
			.should("have.value", "08 2020 31");
		cy.get('input[type="hidden"]')
			.last()
			.should("have.value", "09 2020 05");
	});
});
...
describe("disable prop", () => {
	...
	it("RegExp in time type", () => {
		...
		cy.get(".pdp-time .pdp-moment > div")
			...
			.should("have.attr", "class")
			.and("match", /disabled/);
		cy.get(".pdp-time .pdp-moment button")
			.eq(3)
			.as("momentButtonThree");
		cy.get(".pdp-time .pdp-moment > div")
			.first()
